...
//var mapDiv = $('[id$="map_canvas"]').attr('id');
var shape = [];
...
$(document).ready(function() {
  console.log('PBS_AAAP_IdentifyWinnerSearch.js document.ready() starting...');
  shape = SF.Shape;
  props = SF.Props;

  console.log('PBS_AAAP_IdentifyWinnerSearch.js SF.Shape:' + JSON.stringify(shape));
  console.log('PBS_AAAP_IdentifyWinnerSearch.js SF.Props:' + JSON.stringify(props));

  $(selectCellSelector).css('display', 'none');

  if (shape) {
    console.log('PBS_AAAP_IdentifyWinnerSearch.js document.ready() showing "Render Properties to Map" button...');
    $('#btn_rend_props').show();
  } else {
    console.log('PBS_AAAP_IdentifyWinnerSearch.js document.ready() hiding "Render Properties to Map" button...');
    $('#btn_rend_props').hide();
  }

  //console.log('PBS_AAAP_IdentifyWinnerSearch.js document.ready() hiding map div on page...');
  //$(mapDiv).hide();
  ...
  if (region == 'National Capital Region') {
    console.log('PBS_AAAP_IdentifyWinnerSearch.js document.ready() enabling metro area field...');
    $('#metroArea').prop('disabled', false);
  } else {
    console.log('PBS_AAAP_IdentifyWinnerSearch.js document.ready() disabling metro area field...');
  ...
  if (rlpAfter2016 == 'true') {
    console.log('PBS_AAAP_IdentifyWinnerSearch.js document.ready() disabling HVAC field...');
    $('input.HVACOT').attr('checked', false);
    $('input.HVACOT').attr('disabled', true);
  } else {
    console.log('PBS_AAAP_IdentifyWinnerSearch.js document.ready() enabling HVAC field...');
...
function snapshot(Id) {
            ...
            .find('a:visible')
            .each(function() {
              let dUrl = decodeURIComponent($(this).attr('href'));
              console.log(
                'a03' + dUrl.split('/a03').pop().substring(0,15)
              );
              off.push(
                'a03' + dUrl.split('/a03').pop().substring(0,15)
              );
  ...
  Visualforce.remoting.Manager.invokeAction(
    ...
    function(result, event) {
      if (event.status) {
        ...
        if(strURL.includes("javascript")){
          strURL = strURL.replaceAll("javascript:srcUp('","");

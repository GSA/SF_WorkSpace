...
function removeLayer(layer) {
  if (AAAPGMAP.layers[layer]) {
    AAAPGMAP.map.removeLayer(AAAPGMAP.layers[layer]);
...
function loadLessorLayer() {
  ...
  require([
    ...
    var infoTemplate = new InfoTemplate('<div style="height:0px;"></div>', '');
    ...
    AAAPGMAP.layers.LessorLayer.on('update-start', function(event) {
      container.LoadingOverlay('show', {
        imagePosition: 'center center',
        maxSize: '75px'
      });
    ...
    AAAPGMAP.layers.LessorLayer.on('update-end', function(event) {
      ...
      console.log('Lessor Layer', attributes);
      ...
      for (i = 0; i < body.length; i++) {
        body[i] =
          '<tr><td>' + body[i] + ':</td><td>${' + body[i] + '}</td></tr>';
      }

      AAAPGMAP.layers.LessorLayer.infoTemplate.setContent(
    ...
    AAAPGMAP.map.addLayer(AAAPGMAP.layers.LessorLayer);
...
function loadFloodZoneLayer() {
  ...
  require([
    ...
    AAAPGMAP.layers.FloodZoneLayer = new ArcGISDynamicMapServiceLayer(
    ...
    AAAPGMAP.layers.FloodZoneLayer.on('update-start', function(event) {
      container.LoadingOverlay('show', {
        imagePosition: 'center center',
        maxSize: '75px'
      });
    ...
    AAAPGMAP.layers.FloodZoneLayer.on('update-end', function(event) {
      container.LoadingOverlay('hide');
    });

    AAAPGMAP.map.addLayer(AAAPGMAP.layers.FloodZoneLayer);

    // Flood zones are not shown below zoom 13.
    AAAPGMAP.setExtent();
  });
}
...
function streetViewToggle() {
  AAAPGMAP.map.setBasemap('topo-vector');

...
console.log('ArcGIS_Interface JS starting up...');
console.log('Pre-initialization of AAAP.form starting...');
let AAAP = {};
AAAP.form = {
    container: null,
    location: {
...
console.log('Initializing AAAP.map to null...');
AAAP.map = {
    container: null,
    service: null
};
console.log('Initializing AAAP.table to null...');
AAAP.table = {
    container: null
}
console.log('Done with pre-initializing, about to start AAAP.form.initialize() function...');
//Form Init
AAAP.form.initialize = function (initialLoad) {
    //Initialize the form.
    console.log('AAAP.form.initialize function starting... initialLoad flag:', initialLoad);
    if (initialLoad) {
	console.log('AAAP.form.initialize initialLoad if-statement logic starting...');
        // Set form
	console.log('Setting form container to the offer...');
        this.container = $('[id$="newOffer"]');
        // Disable autocomplete on form level.
        this.container.attr('autoComplete', 'off');//this is not working
	console.log('About to execute this.initSearchButtons() function...');
        this.initSearchButtons();
	console.log('About to execute this.initSearchLocation() function...');
	    ...
	    console.log('ArcGIS v4.19 Interface JS file accept-address onClick starting...');
	    ...
	    console.log('ArcGIS v4.19 Interface JS file show-map() onClick starting...');
	    ...
	    console.log('ArcGIS v4.19 Interface JS file show-latlon() onClick starting...');
    ...
    console.log('After initialLoad logic...');
    ...
    console.log('Done with AAAP.form.initialize() function.');
...
AAAP.form.clearMapMarkers = function () {
    ...
    console.log('ArcGIS v4.19 Interface JS file clearMapMarkers() starting...');
...
AAAP.form.initSearchButtons = function () {
    ...
    console.log('ArcGIS v4.19 Interface JS file initSearchButtons() starting...');
    ...
    $('#submit-address').on('click', function () {
        ...
        AAAP.map
            ...
            .catch(function (error) {
		//Todd Brown replaced 06/2023 for story SFWS-2617 with Community conversion
                //console.error(error);//error here if nothing is input into address box. Shouldn't there be a message?
                if (address != null && address.length > 0) {
                    console.log('A web service error occurred on click of submit link for address text field...');
                    console.error(error);
		    alert('An error occurred attempting to retrieve and verify address. Please refresh this page and try again.  If complications persist, please email LOP.Help@gsa.gov or call 1-866-450-6588 and select option 7. ');
                } else {
		    alert('Please enter a street address and try again.  If complications persist, please email LOP.Help@gsa.gov or call 1-866-450-6588 and select option 7. ');
                }                 
            });
    });
    ...
    $('#submit-coordinates').on('click', function () {
        ...
        console.log('ArcGIS v4.19 Interface JS file submit-coordinates() onClick starting...');
...
AAAP.form.initSearchLocation = function () {
    ...
    console.log('ArcGIS v4.19 Interface JS file initSearchLocation() starting...');

    var list = $('.suggestive-search--address-results');
    var last = 0;
    //var startTimeStamp = null;
    //var errorRepeatIntervalMillis = 20000; //20 seconds
        ...
        .on('keyup', function () {
            ...
            if (value.length > 3) {
                ...
                AAAP.map
                    ...
                    .catch(function (err) {
                        console.log('Error occurred in callouts for suggestive-search...');
                        console.error(err);
			...
			//    startTimeStamp = Date.now();
                        //} else {
                        //    if (Date.now() - startTimeStamp > errorRepeatIntervalMillis) {
                        //        startTimeStamp = null; //reset timer for next error alert pop-up
                        //    }
                        //}
                    });
            } else {
...
AAAP.form.setLocation = function () {
    ...
    console.log('ArcGIS v4.19 Interface JS file setLocation() starting...');
...
AAAP.form.rlpChanged = function () {
    ...
    console.log('ArcGIS v4.19 Interface JS file rlpChanged() starting...');

    var rlp;
    /*
    rlp = $('select[id$="chooseRLP"]')
        .children(':selected')
        .text();
    */
    console.log('Var rlp: ' + rlp);
    var region;
    /*
    region = rlpRegionMap[rlp];
    */
    console.log('Var region: ' + region);
    if (
        region === 'Census' &&
        !window.confirm(
    ...
    console.log('In rlpChanged() function and about to call final command AAAP.form.initialize() with false paramater...');
...
AAAP.form.isInRlp = function (fullAddress, state, city, subregion) {
    ...
    console.log('ArcGIS v4.19 Interface JS file isInRlp() starting...');
...
AAAP.map.initialize = function () {
    ...
    console.log('ArcGIS v4.19 Interface JS file AAAP.map.initialize() starting...');
    ...
    this.getService()
        ...
        .then(function (instance) {
            console.log('Map service initialized with topo-vector basemap.', instance);
            $('[id$="view-street"]').css('font-weight', 'bold');
            $('[id$="view-street"]').css('background-color', 'silver');
        })
        .catch(function (err) {
            //console.log(err);
        });

    // Attach event handler for toggling "street" topo-vector view.
    $('#view-street').on(
        'click',
        function () {
	    console.log('view-street button clicked.');
            this.setView('topo-vector');
        }.bind(this)
    );

    // Attach event handler for toggling "satellite" hybrid view.
    $('#view-satellite').on(
        'click',
        function () {
	    console.log('view-satellite button clicked.');
            this.setView('hybrid');
        }.bind(this)
    );
	...
	console.log('layer-flood button clicked.');
	...
	console.log('layer-seismic button clicked.');
        if ($(this).hasClass('active')) {
            $(this).removeClass('active');
    ...
    /*
    // Attach event handler for CBD layer.
    ...
    // Attach event handler for transit layer.
...
AAAP.map.setService = function (container) {
    ...
    console.log('ArcGIS v4.19 Interface JS file AAAP.map.setService() starting...');
...
AAAP.map.getService = function () {
    ...
    console.log('ArcGIS v4.19 Interface JS file AAAP.map.getService() starting...', this.service);
...
AAAP.map.setView = function (view) {
    ...
    console.log('ArcGIS v4.19 Interface JS file AAAP.map.setView() starting...');
    this.service.setView(view);

    switch (view) {
        case 'hybrid': 
            $('[id$="view-street"]').css('font-weight', 'normal');
            $('[id$="view-street"]').css('background-color', '#ffffff');
            $('[id$="view-satellite"]').css('font-weight', 'bold');
            $('[id$="view-satellite"]').css('background-color', 'silver'); 
	    break;
        default:
            $('[id$="view-street"]').css('font-weight', 'bold');
            $('[id$="view-street"]').css('background-color', 'silver');
            $('[id$="view-satellite"]').css('font-weight', 'normal');
            $('[id$="view-satellite"]').css('background-color', '#ffffff');
    }
...
AAAP.table.rowClick = function () {
    ...
    //console.log('In AAAP.table.rowClick function.', addr);
	...
	console.log('In AAAP.table.rowClick function, about to call AAAP.form.isInRlp() with address.', addr); 
        var inRlp = AAAP.form.isInRlp(null, addr.state, addr.city, addr.county);
	//console.log('inRlp?', inRlp);
        if (!inRlp) {
            alert(
	    ...
	    console.log('Address is within RLP so about to hide map container and show "other_bldg_data" and "show-map" fields...');
            AAAP.map.container.hide();
            $('[id$="other_bldg_data"]').show();
            $('#show-map').show();

            AAAP.setAddressFields(addr);
	    console.log('About to set var cds to AAAP.map.getService().layers.CongressionalDistrictLayer...');
            var cds = AAAP.map.getService().layers.CongressionalDistrictLayer;

            //JK added this logic because layers are currently commented out as of June 2021
	    console.log('Var cds:', cds);
            if (cds !== undefined) {
